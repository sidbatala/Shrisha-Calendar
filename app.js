...
(function() {
    ...
    if (calendarTable) {
        ...
        // Internal state: always store BS year/month (for BS mode) and AD year/month (for AD mode)
        let bsYear = 2082;
        let bsMonth = 7; // 0-indexed (0 = Baishakh)
        let adYear = 2026;
        let adMonth = 7; // 0-indexed (0 = January)
        ...
        function populateSelectors() {
            if (showAD) {
                // AD mode: populate with AD months and years
                ...
                // AD years range: 1943 to 2042
                const adYears = [];
                for (let y = 1943; y <= 2042; y++) adYears.push(y);
                yearSelector.innerHTML = adYears.map(y => `<option value="${y}">${y}</option>`).join('');

                // Set selected values: try to keep old selection if valid, otherwise use current adYear/adMonth
                ...
                // BS mode: populate with BS months and years
                const years = getAvailableYears();
        ...
        // ---- Get AD month/year from current state ----
        ...
        // ---- Get BS month/year from AD selection ----
        function getBSFromADMonthYear(adYear, adMonth) {
            // Find the BS month that contains the first day of the AD month
        ...
        // New: get BS span for an AD month
        ...
        function generateCalendar(month, year, todayBs) {
            // month and year are in BS always (for BS mode) or AD (for AD mode)
            ...
            // Compute today's AD date for robust highlighting
            const now = new Date();
            ...
            if (displayMode === 'BS') {
                ...
                // ---- AD Calendar ----
                // Use the stored AD year and month directly
                const adYearLocal = adYear;
                const adMonthLocal = adMonth;
                // Use pure Gregorian calculations from DateCalc
                ...
                for (let row = 0; row < 6; row++) {
                    ...
                    for (let col = 0; col < 7; col++) {
                        ...
                        // Check if this AD date is today (robust)
        ...
        function openDayPopup(dateObj, bsYear, bsMonth, bsDay) {
            ...
            // Reverse date sizes when in AD mode
            if (showAD) {
            ...
            if (festivals.length) {
                ...
                popupFestivals.innerHTML = html; // removed icon
            } else {
                popupFestivals.textContent = "";
            }

            popupFullPanchangLink.href = "#";
        ...
        function populateMonthYearAndRender() {
            ...
            // If in AD mode, make sure dropdown values are in sync
            if (showAD) {
                yearSelector.value = adYear;
                monthSelector.value = adMonth;
            } else {
        ...
        function initToggle() {
            toggleModeBtn.addEventListener('click', function() {
                ...
                // When toggling, update the internal state and dropdowns
                if (showAD) {
                    // Set AD state to current date
                    const now = new Date();
                    adYear = now.getFullYear();
                    adMonth = now.getMonth();
                    // Compute BS from this AD date for panchang/festivals
                    const bs = adToBs(now);
                    if (!bs.outOfRange) {
                        bsYear = bs.year;
                        bsMonth = bs.month;
        ...
        function initCalendar() {
            ...
            console.log("Shrisha Panchang loaded with fixed Kathmandu location.");
        ...
        console.log("Calendar table not found – skipping UI initialization.");
    }
