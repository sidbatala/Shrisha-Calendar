...
(function () {
    ...
    const CONTACT_EMAIL = 'sidbatala@gmail.com';
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok && !payload.errors) {
                    showStatus('success',
                        '<i class="fas fa-check-circle" aria-hidden="true"></i> Thank you! Your message has been sent. We\'ll get back to you within 24 hours.');
                    form.reset();
                    if (charCount) charCount.textContent = `0 / ${MAX_MESSAGE}`;
                    clearErrorAll();
                } else {
                    showStatus('error',
                        `<i class="fas fa-exclamation-circle" aria-hidden="true"></i> Could not send your message. Please try again or email us at ${CONTACT_EMAIL}.`);
                }
            } catch {
                showStatus('error',
                    '<i class="fas fa-exclamation-circle" aria-hidden="true"></i> Network error. Please check your connection and try again.');
            } finally {
