...
(function () {
    ...
    const REFRESH_MS = 5 * 60 * 1000;   // auto-refresh current date every 5 min
    const MAX_LOOKBACK_DAYS = 7;         // if selected date has no data, look back this many days
    const MIN_DATE = '2010-01-01';       // earliest selectable AD date
    ...
    let pickerMode = 'AD';           // 'AD' or 'BS'
    let bsPickerSyncFn = null;       // set by initBsPicker(), used to keep pickers in sync
    ...
    function flagImgHtml(code, cls) {
        ...
        if (!file) return '';
        return `<img src="assets/flags/${file}" alt="${code}" class="${cls}"
    ...
    function initBsPicker() {
        ...
        // Reference bounds in AD
        const today    = startOfDay(new Date());
        const minDate  = new Date(MIN_DATE + 'T00:00:00');

        // ---- Populate BS years (only those whose AD range is within [MIN_DATE, today]) ----
        const years = getAvailableYears();
        ...
        // ---- Populate BS days based on selected year/month ----
        ...
        function syncFromAd(dateObj) {
            ...
            if (bs.outOfRange) return;
            // Ensure the year is in the valid list
        ...
        function syncFromBs() {
            ...
            if (clamped) {
                // Snap the BS picker to the clamped date
        ...
        // Expose the sync function so navigation can update the BS picker
        bsPickerSyncFn = syncFromAd;
    }

    // ============================================================
    // API FETCH (historical date support)
    // ============================================================
    ...
    // ============================================================
    // CURRENCY CONVERTER  (defaults to 1 USD → NPR)
    // ============================================================
